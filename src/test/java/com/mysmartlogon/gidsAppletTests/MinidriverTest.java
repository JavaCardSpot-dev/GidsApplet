...
//				"DF 20 10 A7 F5 C7 42 89 55 7B C0 7E 9A 9D E3 DD BD 21 D7 90 00");
    ...
    */    
    ...
    public void importOneRSA2048Key() {
        ...
        // Create keyfile: Create an elementary file(parameters in TLV format)
        // 00 E0 00 00 params
        ...
        if (physicalCard != null && physicalCard.getProtocol().equals("T=1")) { // T=1 protocol (receive everything at once)
            execute("00 CB 3F FF 0A 70 08 84 01 81 A5 03 7F 49 80 00",
                    "7F 49 82 01 09 81 82 01 00 AE A1 7E EC 31 7D 53 86 0E CD 2D F7 1A E0 62 5B CD 06 F9 A3 A2 33 48 72 C3 17 07 F3 26 2E F6 33 91 C5 18 90 EC 8E AD 77 6D C5 7B 74 4B 6C 1F D6 43 C4 4E 65 D5 19 2B 9A 69 63 9A FF 95 E7 03 D6 E8 F8 83 1E A0 6B BD 29 71 00 A8 FE 98 2C 8D 6F DB 40 27 AA C3 5A F5 D3 13 AC F5 04 75 90 8E F1 5F 64 AE 53 EC 43 12 CC 51 74 3F 40 F4 F3 0B DF B0 28 CA 9D 1D 34 B4 A1 0C BE E8 BD 2C 23 40 3C 90 7B 21 B7 5D D2 DD F0 2A 73 34 62 2D CE 6E 40 FE 39 31 CD D2 DB 97 DD 63 21 6D 61 BB 9A A1 7E F7 CC 90 0E C8 C9 E0 79 CF CA 46 1F 67 48 28 66 73 80 7E 92 7B 9D 7E 80 3A 80 D6 15 8D 7C 62 ED 6A 1B BC BF 1B DD D1 7A 7C 26 59 FB C6 8F DB 0F 8E 56 A4 6F 58 5E D8 46 F6 2F 8A FE BD 36 30 EF 2C D9 81 37 AF 0D F4 CA D0 74 E8 9B E0 D9 A0 43 39 DF 7B CF 02 BE D9" +
            ...
            // Get response after reading public key
            execute("00 C0 00 00 0E",
                    "92 8A 11 58 AB 73 FA A3 59 82 03 01 00 01 90 00");
        }
        // Sign some data : 00 22 9E 9A lendata 00
        // MANAGE SECURITY ENVIRONMENT  22
        // PERFORM SECURITY OPERATION   2A
        // 00 22 41 operation 06 80 01 algo 84 01 keyref Set the key and algorithm for a crypto operation
        execute("00 22 41 B6 06 80 01 57 84 01 81");
        // 00 2A 9E 9A DER-encoded digest information (PKCS 1.5 only) || data to be signed (precomputed hash value) --> Plain signature without furtherTLV coding
